    ...
    <script type="text/javascript">
        ...
        function OnDsMainItemChanged(s, r, c, v) {
            if (c == "pay_recv_status") {
                ...
                if (dsMain.GetItem(0, "member_flag") != "8") {
                    dsMain.SetItem(0, "nonmember_detail", "");
                }
            } else if (c == "tax_amt" || c == "vat_amt") {
                var item_amt = 0;
                for (var i = 0; i < dsList.GetRowCount(); i++) {
                    item_amt += Gcoop.ParseFloat(dsList.GetItem(i, "itempay_amt"));
                }
                item_amt = item_amt.toFixed(2)
                var vat_amt = Gcoop.ParseFloat(dsMain.GetItem(0, "vat_amt")).toFixed(2);
                var tax_amt = Gcoop.ParseFloat(dsMain.GetItem(0, "tax_amt")).toFixed(2); 
                if (Gcoop.GetEl("HdincludeVat").value == "0") {
                    item_amt = Gcoop.ParseFloat(item_amt)+ Gcoop.ParseFloat(vat_amt); 
                } 
                item_amt = Gcoop.ParseFloat(item_amt) - Gcoop.ParseFloat(tax_amt); 
                dsMain.SetItem(0, "item_amtnet", item_amt);
                dsList.SetItem(0, "itempayamt_net", item_amt);
                dsList.SetItem(0, "tax_amt", tax_amt);
                dsList.SetItem(0, "vat_amt", vat_amt);
            }
        }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlace" runat="server">
    ...
    <asp:HiddenField ID="HdincludeVat" Value="0" runat="server" />
</asp:Content>
