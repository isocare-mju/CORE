    ...
    <script type="text/javascript">
        ...
        function OnButtonClicked(s, row, oName) {
            if (oName == "b_account") {
                ...
                if (recpPayType == "DTR" || recpPayType == "DTW" || recpPayType == "WTR") {
                ...
                Gcoop.OpenIFrame2Extend("900", "650", "w_dlg_dp_signature.aspx", "?dpdept_no=" + dpdept_no);
            } 
            return 0;
        }
        ...
        function OnDwChequeItemChanged(s, r, c, v) {
            ...
            if (c == "cheque_no") {
                ...
                if (objDwMain.GetItem(1, "deptwith_flag") != "/") {
                    ...
                    if (chequeTotal > objDwMain.GetItem(1, "deptslip_netamt")) {
                        objDwCheque.SetItem(r, "cheque_amt", 0.00);
                        alert('ยอดถอนด้วยเช็คมากกว่ายอดปิดบัญชี');                       
                    }
                }
                objDwMain.AcceptText();
                s.SetItem(r, c, v);
        ...
        function GetDeptSlipNoFromDlg(deptSlipNo, deptSlipNetAmt) {
            ...
            //กรณีถอนโอนบัญชี 1 ก้อน ไม่ให้เปลี่ยนยอดถอนโอนมาฝาก
            if (objDwMain.GetItem(1, "recppaytype_code") == "DTW") {
                DisabledTable(objDwMain, "deptslip_amt", "DwMain", null);
            }
            objDwMain.AcceptText();
        }
        ...
        function Validate() {
            ...
            //bee cls_chq
            if (objDwMain.GetItem(1, "deptwith_flag") == "/" && objDwMain.GetItem(1, "cash_type") == "CHQ") {
                var chequeTotal = 0;
                for (i = 0; i < objDwCheque.RowCount(); i++) {
                    chequeTotal += Gcoop.ParseFloat(objDwCheque.GetItem(i + 1, "cheque_amt"));
                }
                if (chequeTotal != objDwMain.GetItem(1, "deptslip_netamt")) {
                    alert('ยอดถอนด้วยเช็คไม่เท่ากับยอดปิดบัญชี');
                    return false;
                }
            }
            if (objDwMain.GetItem(1, "recppaytype_code") == "DTW") {
                if (objDwMain.GetItem(1, "refer_slipno") == null || objDwMain.GetItem(1, "refer_slipno") == "") {
                    alert('กรุณาเลือกอ้างอิงรายการฝากโอน');
                    return false;
                }
            }
